...
# Plot the transformed vector field with small black arrows
# plt.quiver(x, y, u - x, v - y, color='k', angles='xy', scale_units='xy', scale=11, alpha=0.5)
c = plt.streamplot(x, y, u - x, v - y, density=1, linewidth=None, color='#A23BEC')
c.lines.set_alpha(0.5)
c.arrows.set_alpha(0.5)
# plt.quiver(x, y, u - x, v - y, angles='xy', scale_units='xy', scale=5, alpha=0.5, color='k', width=0.002)
...
The eigenvector with the smallest eigenvalue is the direction in which the matrix $A$ shrinks the space the most.
...
ax.streamplot(x, y, u_imag, u_real-x, v_real-y, v_imag-u_imag, color='b')

arrow_prop_dict = dict(mutation_scale=2, arrowstyle='-|>', shrinkA=0, shrinkB=0)

print(eigenvectors[1][i].imag)
...
def plot_3d_quiver(x, y, z, u, v, w, ax):
    ax.quiver(x, y, z, u, v, w, colors = 'b', alpha=0.1, length = .2, arrow_length_ratio = 0.005)
    return ax
...
ax = plot_3d_quiver(x, y, u_imag, u_real, v_real, v_imag, ax)

arrow_prop_dict = dict(mutation_scale=10, arrowstyle='-|>', shrinkA=0, shrinkB=0)

# Plot 3D eigenvectors
for c, i in zip(['b', 'g'], [0, 1]):
    print(eigenvectors[0][i].real, 0, eigenvectors[1][i].real, eigenvectors[0][i].imag)
    a = Arrow3D([0, eigenvectors[0][i].real], [0, eigenvectors[1][i].real], 
            [0, eigenvectors[1][i].imag], color=c, **arrow_prop_dict)
    ax.add_artist(a)
