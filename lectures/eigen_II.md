...
一个 $n \times n$ 的非负矩阵 $A$ 如果满足 $A + A^2 + A^3 + \cdots \gg 0$，则被称为**不可约**的，其中 $\gg 0$ 表示 $A$ 的每个元素都严格为正。
...
左特征向量在下文中将扮演重要角色，包括在马尔可夫假设下的动态模型中的随机稳态。

如果 $w$ 是 $A^\top$ 的右特征向量，那么 $w$ 被称为 $A$ 的左特征向量。

换句话说，如果 $w$ 是矩阵 $A$ 的左特征向量，那么 $A^\top w = \lambda w$，其中 $\lambda$ 是与左特征向量 $w$ 相关的特征值。
...
对于任何这样的矩阵 $A$，佩龙-弗罗贝尼乌斯定理刻画了主特征值及其对应特征向量的某些特性。
...
5. 不存在其他与 $r(A)$ 相关的正特征向量（除了 $v$ 的标量倍数）。

（关于原始矩阵的佩龙-弗罗贝尼乌斯定理的更多内容将在 {ref}`下文 <prim_matrices>` 中介绍。）
```

（这是该定理的一个相对简单的版本——更详细的内容请参见[这里](https://en.wikipedia.org/wiki/Perron%E2%80%93Frobenius_theorem)）。

我们将在下文中看到该定理的应用。

让我们用之前见过的一个简单[例子](mc_eg1)来建立对这个定理的直觉理解。
...
这里的 $A$ 也是一个原始矩阵，因为对于某个 $k \in \mathbb{N}$，$A^k$ 处处为正。
...
```{prf:Theorem} 佩龙-弗罗贝尼乌斯定理的连续性
:label: con-perron-frobenius

如果 $A$ 是原始矩阵，那么：

6. 对于 $A$ 的所有不同于 $r(A)$ 的特征值 $\lambda$，不等式 $|\lambda| \leq r(A)$ **严格成立**，并且
7. 当 $v$ 和 $w$ 被归一化使得 $w$ 和 $v$ 的内积等于1时，我们有：当 $m \rightarrow \infty$ 时，$r(A)^{-m} A^m$ 收敛于 $v w^{\top}$。矩阵 $v w^{\top}$ 被称为 $A$ 的**佩龙投影**。
```
...
我们还可以验证佩龙-弗罗贝尼乌斯定理在这些随机矩阵中所暗示的其他性质。
...
:label: eig_ex1
```
[华西里·列昂惕夫](https://en.wikipedia.org/wiki/Wassily_Leontief)
...
1. 由于$A$是一个非负不可约矩阵，求$A$的佩龙-弗罗贝尼乌斯特征值。
