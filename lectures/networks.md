...
在研究各种网络时，一个反复出现的话题是不同节点的相对"中心度"或"重要性"。
...
在接下来的内容中，**中心度度量**将每个加权有向图与一个向量$m$关联起来，其中$m_i$被解释为节点$v_i$的中心度（或排名）。
...
```{code-cell} ipython3
iG7 = [G7.in_degree(v) for v in G7.nodes()]   #计算入度中心度
...
虽然入度和出度中心度计算简单，但它们并不总是有用。

在{numref}`financial_network`中，几乎每个节点之间
都存在边，所以基于入度或出度的中心度排名无法有效区分国家。
...
该图的**特征向量中心度**定义为满足以下方程的$n$维向量$e$：
...
注意这个定义的递归性质：节点$i$的中心度与它所连接的所有节点的中心度成正比，权重就是从$i$到这些节点的连接强度。

一个节点的中心度排名会很高，如果它：
1. 连接到许多其他节点，
2. 这些连接的权重较大，并且
3. 它所连接的节点本身也具有较高的中心度。

在后续章节中，我们将在生产网络的背景下深入探讨特征向量中心度的应用。

在生产网络中，特征向量中心度高的部门通常是关键*供应商*，它们为经济中的多个重要部门提供基础投入。
...
让我们为{numref}`sample_gph_1`中生成的图计算特征向量中心度。
...
虽然节点 $2$ 和 $4$ 具有最高的入度中心度，但我们可以看到节点 $1$ 和 $2$ 具有最高的特征向量中心度。
...
特征向量中心度的一大优点是它不仅考虑节点本身的重要性，还考虑了与之相连的邻居节点的重要性。

谷歌著名的PageRank算法实际上就是特征向量中心度的一种变体，用于为网页建立排名系统。

这一算法的核心思想是：来自重要网页（按度中心度衡量）的链接比来自不太重要网页的链接更有分量。

### 卡茨中心度

特征向量中心度存在一个潜在问题：当$r(A)$为零时，$1/r(A)$无法定义。

基于这个原因以及其他一些考虑，研究人员常常转向另一种网络中心度度量——卡茨中心度。

对于一个带权有向图，我们选择参数$\beta$使其位于$(0, 1/r(A))$区间内，然后将**卡茨中心度**定义为满足以下方程的向量$\kappa$：
...
卡茨中心度的基本思想与特征向量中心度相似：如果节点$i$被那些本身具有高中心度的节点所链接，那么它也会获得较高的中心度评分。

只要选择$\beta$满足$0 < \beta < 1/r(A)$，卡茨中心度就能保证有限且唯一，因为此时$r(\beta A) < 1$。
...
参数$\beta$的作用是确保中心度度量$\kappa$保持有限。

当谱半径$r(A)<1$时，我们通常采用$\beta=1$作为卡茨中心度的默认计算参数。
...
一些页面拥有高**枢纽中心度**，这意味着它们链接到许多有价值的信息源（比如新闻聚合网站）。

而另一些页面则具有高**权威中心度**，表示它们本身包含有价值的信息，这通常体现在有多少重要网站链接到它们（例如知名新闻机构的官方网站）。

这些概念同样适用于经济网络分析，尽管可能使用不同的术语。

我们前面讨论的特征向量中心度和Katz中心度实际上测量的是枢纽中心度。
（如果一个节点指向许多高中心度的节点，那么它自身的中心度也会很高。）

如果我们更关注权威中心度，可以使用相同的定义方法，只需将邻接矩阵转置即可。

这种方法之所以有效，是因为转置操作会反转所有箭头的方向。

（这样一来，如果一个节点接收到来自多个高中心度节点的链接，它的中心度就会变高。）

例如，对于邻接矩阵为$A$的加权有向图，其**基于权威的特征向量中心度**是满足以下方程的向量$e$：
...
下图展示了{numref}`financial_network`中国际信贷网络的基于权威的特征向量中心度排名。
...
1. 使用 `Networkx` 绘制图 $G$。
2. 找出 $G$ 的相关邻接矩阵 $A$。
3. 使用上面定义的函数计算 $G$ 的入度中心度、出度中心度和特征向量中心度。
...
```{code-cell} ipython3
oG = [G.out_degree(v) for v in G.nodes()]   # 计算入度中心度
...
```{code-cell} ipython3
e = eigenvector_centrality(A)   # 计算特征向量中心度
n = len(e)
