...
本讲座部分内容取自 https://networks.quantecon.org 一书，但本讲座的层次更加基础入门。
...
在下面的讨论中，我们将学习如何量化这类概念。

### 例子：马尔可夫链

在我们之前学习的{ref}`马尔可夫链 <mc_eg2>`讲座中，我们分析了一个商业周期模型，其中的状态包括
...
网络科学的理论基础主要来自数学中的[图论](https://baike.baidu.com/item/%E5%9B%BE%E8%AE%BA/1433806)这一重要分支。
...
（表示箭头的一个巧妙方法是记录箭头的起点和终点位置，这正是边所做的。）

以 {numref}`aircraft_network` 中的飞机出口网络为例：

* $V$ 包含数据集中的所有国家。

* $E$ 包含图中所有的箭头，每个箭头表示某个国家向另一个国家出口了一定数量的飞机。
...
在这些图中，箭头（边）可以理解为表示在给定时间单位内的正的转移概率。
...
当节点 $u=v$，或者存在一系列从 $u$ 到 $v$ 的边时，我们说节点 $v$ 从节点 $u$**可达**。

* 这种关系记为 $u \to v$

（直观地说，这表示有一系列箭头从 $u$ 指向 $v$。）

举个例子，假设我们有一个表示生产网络的有向图，其中：

* $V$ 中的元素代表不同的工业部门
* 存在边 $(i, j)$ 表示部门 $i$ 为部门 $j$ 提供产品或服务

在这种情况下，$m \to \ell$ 意味着部门 $m$ 是部门 $\ell$ 的上游供应商。

当 $u \to v$ 且 $v \to u$ 同时成立时，我们说节点 $u$ 和 $v$ **相通**。

如果图中的所有节点都相通，我们称这个图是**强连通的**。

例如，{numref}`poverty_trap_1` 是强连通的，
而在 {numref}`poverty_trap_2` 中，由于富人节点无法从穷人节点到达，所以它不是强连通的。

我们可以先用Networkx构造这些图，然后使用 `nx.is_strongly_connected` 来验证这一点。
...
我们现在引入加权图的概念，在这类图中，每条边都被赋予一个权重（数值）。

### 各国间私人信贷流动

为了更好地理解这个概念，请看下图，它展示了不同国家私人银行之间的资金流动（即贷款）情况，数据按来源国分组。
...
图中从日本到美国的箭头代表日本银行对所有在美国注册银行的总债权，这些数据来自国际清算银行（BIS）的统计。

图中每个节点的大小随着其他所有节点对该节点的外国债权总额的增加而增大。

箭头的粗细则与它们所代表的外国债权规模成正比。

请注意，在这个网络中，几乎对于每一对 $u$ 和 $v$（即网络中几乎每个国家），都存在一条边 $(u, v)$。

（实际上还有更多较小的箭头，为了图表清晰我们已将其省略。）

因此，仅凭一个节点到另一个节点之间是否存在边，并不能提供太多有意义的信息。

要理解这个网络，我们不仅需要记录信贷流动的存在与否，还需要记录流动的规模。

用于记录这些信息的正确数据结构是"加权有向图"。

+++

### 定义

**加权有向图**是一个有向图，在此基础上我们添加了一个**权重函数** $w$，它为每条边赋予一个正数。

上图展示了一个加权有向图，其中权重表示资金流动的规模。

下图显示了一个加权有向图，箭头表示由此导出的有向图的边。
...
在这个例子中，你可以将箭头上的数字理解为一个家庭在（比如说）一年内的转移概率。

我们可以看到，一个富裕家庭在一年内有10%的概率变为贫困。

## 邻接矩阵

我们还可以用另一种方式来表示权重，事实证明这种方式在数值计算中非常方便，那就是使用矩阵。

假设我们有一个加权有向图，其节点为 $\{v_1, \ldots, v_n\}$，边集为 $E$，权重函数为 $w$，则该图的**邻接矩阵**定义为
...
一般来说，任何非负的 $n \times n$ 矩阵 $A = (a_{ij})$ 都可以被
视为一个加权有向图的邻接矩阵。

为构造这个图，我们令 $V = 1, \ldots, n$，并取边集 $E$ 为
所有满足 $a_{ij} > 0$ 的 $(i,j)$。

对于权重函数，我们对所有边 $(i,j)$ 设 $w(i, j) = a_{ij}$。

我们把这个图称为由 $A$ 导出的加权有向图。

## 性质

考虑一个邻接矩阵为 $A$ 的加权有向图。

设 $a^k_{ij}$ 为 $A^k$（即 $A$ 的 $k$ 次幂）的第 $i,j$ 个元素。
...
当 $k=1$ 时，上述结果是显然的，一般情形的证明可以
在 {cite}`sargent2022economic` 中找到。

现在回忆一下我们在特征值讲座中学到的内容，一个
非负矩阵 $A$ 被称为{ref}`不可约的<irreducible>`，如果对于每一对 $(i,j)$，都存在一个整数 $k \geq 0$ 使得 $a^{k}_{ij} > 0$。

由前面的定理，不难得到下面的结果（详见
{cite}`sargent2022economic`）。
...
再举一个例子，我们可以设想一个生产网络，其中某个部门
的重要性取决于它所供应的部门的重要性。

这与前面的例子顺序相反：现在，某个
节点的重要性取决于*它所链接到*的其他节点的重要性。
...
为了简化表示，我们假设图的节点集合$V$就是
整数$1, \ldots, n$。

我们用$r(A)$表示$A$的{ref}`谱半径<neumann_series_lemma>`。
...
注意这个定义的递归性质：节点$i$获得的中心性
与所有节点的中心性之和成正比，且以从$i$流向这些节点的*流动速率*作为权重。
...
在后面，当我们研究生产网络中的需求冲击时，将对特征向量中心性
给出更具体的解释。

我们会看到，在生产网络中，具有高特征向量中心性的部门是重要的*供应商*。
...
在这个排名中，评分较高的国家往往是信贷供应方面的重要参与者。

日本在此指标中位居榜首，而英国和法国等拥有庞大金融体系的国家也表现突出，紧随其后。

特征向量中心性的一大优点是它不仅考虑节点本身的重要性，还考虑了与之相连的邻居节点的重要性。

特征向量中心性的一个变体是谷歌PageRank算法的核心，该算法被用于为网页排名。

其主要原则是：来自重要节点（按度中心性衡量）的链接比来自不重要节点的链接更有价值。

### 卡茨中心性

特征向量中心性的一个问题在于，$r(A)$可能为零，在这种
情况下$1/r(A)$没有定义。

出于这个原因和其他一些考虑，一些研究人员更倾向于使用另一种
网络中心性度量，称为卡茨中心性。

固定 $\beta$ 在 $(0, 1/r(A))$ 区间内，邻接矩阵为 $A$ 的加权
有向图的**卡茨中心性**定义为满足以下方程的向量 $\kappa$：
...
这意味着{eq}`katz_central_vec`有唯一解：

$$
\kappa = (I - \beta A)^{-1} \mathbf{1}
$$

这一结果可由{ref}`诺伊曼级数定理<neumann_series_lemma>`直接推导得出。

参数$\beta$的作用是确保$\kappa$保持有限。

当$r(A)<1$时，我们通常采用$\beta=1$作为卡茨中心性计算的默认值。
...
类似的想法可以（也已经）被应用于经济网络（通常使用不同的术语）。

我们前面讨论的特征向量中心性和卡茨中心性实际上测量的是枢纽中心性。

（如果一个节点指向其他具有高中心性的节点，那么它自身的中心性也会很高。）

如果我们更关注权威中心性，可以使用相同的定义方法，只需将邻接矩阵转置即可。

这种方法之所以有效，是因为转置操作会反转箭头的方向。

（这样一来，如果一个节点接收到来自其他高中心性节点的链接，它的中心性就会变高。）
...
排名靠前的国家通常是那些吸引大量信贷流入，或从其他主要参与者那里获得信贷流入的国家。

在这种情况下，美国作为银行间信贷的目标，明显在排名中占据主导地位。
...
设 $(V, E)$ 是一个有向图，若 $u$ 和 $v$ 相通，则记作 $u \sim v$。
