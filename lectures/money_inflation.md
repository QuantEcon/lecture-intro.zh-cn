...
这个结果将用来证明在讲座 {doc}`unpleasant`中要研究的不愉快的货币主义算术分析中选择一个固定通胀率的合理性.
...
我们将关注参数值和与之相关的实际余额的毛收益率，确保实际余额的需求为正值。根据{eq}`eq:bdemand` 这意味着：
...
这个方程表达了我们系统的动力，使我们可以隔离导致通货膨胀汇聚到我们之前发现的较低稳定状态通货膨胀率 $R_l$ 的逆值的力量。
